...
{r addresses standardize}
library(tidygeocoder)

census_v1 <- geo(df$`Please confirm your GP practice name and postcode`, method = "osm", full_results = TRUE)


library(stringdist)

match_census <- na.omit(census_v1) %>%
  distinct(.keep_all = TRUE)

no_match_census <- apply(is.na(census_v1), 1, any)
no_match_census <- census_v1[no_match_census, ]

matching_v1 <- amatch(no_match_census$address, match_census, nomatch = NA_integer_, matchNA = FALSE, method = "lv", maxDist = 1)
...
knowledge_plot <- ggplot(dfv2, 
  ...
  labs(title = "Distribution of Correct Answers",
       x = "Group",
...
ggplot(model_ok_binary, 
  ...
  labs(title = "Knowledge score impact on opinion based questions",
       y = "Group of questions") +
...
ggplot(percent_stacked, 
  ...
  labs(x = "Precautionary Measure", y = "Cumulative Count", title = "Risk Willingness for Precautionary Measures") +
  
  theme_minimal(base_size = 12) +  
  
  theme(plot.title = element_text(hjust = 0)) +
  
  scale_fill_brewer("Percentage")
```
...
ggplot(model_rk, 
  ...
  labs(title = "Knowledge score impact on reaction based questions",
       y = "Questions") +
...
ggplot(summary_all_glm, aes(y = reorder(cofounder, id),
  ...
  labs(title = "Reactions to having high risk of developing cancer as affected by cofounders",
       y = "Cofounders",
       x = "Odds Ratio") +
...
ggplot(knowledge_cofounders_stacked, 
  ...
  labs(x = "Question", y = "Cumulative Count", title = "Previous genetic experence") +
  
  theme_minimal(base_size = 12) +  
  
  scale_fill_brewer("Answer")
...
ggplot(model_ccf_cut, 
  ...
  labs(title = "Previous genetic test/genetic diagnosis impact on knowledge",
       y = "Questions") +
...
```
CanRisk genetic risk, use for thesis introduction or discussion
future work - improve questions, add age, education level, children
are gps doing enough? genetic counselars
